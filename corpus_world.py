...
Output: corpus_world.sqlite (current state + append-only journal +
...
DB_PATH = ROOT / "corpus_world.sqlite"
...
#
# THE FIXED NUMBERS BELOW ARE TRIPWIRES, NOT REPORTS. DO NOT "MODERNIZE"
# THEM INTO DYNAMIC COUNTS. Elsewhere the standing rule is that a tool
# must COUNT what it REPORTS and never recite a remembered figure — a
# report stating a number it did not measure lies quietly when the world
# moves. That rule is about reports. These are asserts, and their job is
# the opposite: each pins a value computed in an EARLIER sitting so that
# the next fold FAILS LOUDLY if the world has moved without anyone
# meaning it. Converted to len()-of-itself, this file would pass forever
# and prove nothing, and the era's whole proof — that standing rises by
# exactly the operator count while the state hash holds — would be
# disarmed by a tidying commit with nothing going red to announce it.
# Count what you report; assert what must not change; never convert the
# second into the first. (Both windows, 2026-08-30.)
#
