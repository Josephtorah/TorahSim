...
- 2026-09-15 — LAW IS CODE, NARRATIVE IS DATA (the owner, verbatim, in the design thread during an Ezekiel scan, on the record on his "Yes"; relayed here by that thread): (1) "Law is code. Let's keep that in mind." (2) "law is code and narrative is data, variables, and whatever else it does." The reading beside them: a legal passage in ANY book is a procedure to compile, never a description to summarize — the prophets' rules included (Ezekiel 18 and 33 a decision procedure with its base case; 46:1-12 the calendar-keyed gate and the routing rule; 45-46 a schedule; 40-42 construction by reference; 48 a partition; 20's closing filter under the rod); a later book's rule is a spec with its own constants, run and diverge-checked against the Torah's as the tradition did for Ezekiel (Shabbat 13b; Menachot 45a). Narrative is the tape's event stream: its numbers the variables the code reads, its date stamps the clock's markers, its acts the calls that fire the code and the switches that install it, its receipt formulas the closes; the one reverse flow is the tent, where a case with no law produces a rule in its own name. The full text in THE_STEPS.md's compiler-law block.

- 2026-09-15/16 — THE READBACK'S FIRST FORM BUILT (THE DEUTERONOMY WALK 1b, the owner's "Yes 1. Go"): the world reads its own tape back — a retelling
  in the text is a reference row graded against the ledger the run left, an act the retelling alone records is written at the day it happened and
  closed by the run already on the tape, and a disagreement between two tellings stays an open row until a teacher joins the arms. The laws' half
  (the repetition of the laws against the ledger) is the next form, owed at chapter 5 (World/step9/DEUTERONOMY_WALK.md "Sitting 1b").
