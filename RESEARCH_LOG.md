...
(seven cells and the table, 91 asks, 91/91 on its first graded run), law_release_firstborn the 75th daemon (given_at Deut 15:1, installed_by boot; eight
...
the state row 15:4); the tape 10/10 on its THIRD run (CU5's stale count read from the first run's print; the retype's apostrophes; the scan's ground fixed to exclude the daemon's own writes); the gates chain in three passes (Q17 and the dependency gate's five demands from the first pass, the positions table by four workers, then all green) (the register gate DECLARED 98 / DEBT 0 / FAILS 0; the sweep 69/69). THE EXAM'S PERSONS
...
deu_15_reeh_exam_2026-09-23.md — 974 rows, 814 read whole here and 160 carried; its crowns in the map's three AS RUN paragraphs. THE RECORD KEPT: the grep found eight count seats and the
...
(0 code points in 956 rows). THE TIMING TABLE (every step timed — the owner's ask at sitting 10; the machine's seconds per step, the model's reading and writing between them the rest): SITTING 13b 82 timed steps, 4326 machine seconds — RUN A 8 steps 263s, THE DOCKET 23 steps 3s, RUN B 33 steps 1219s, THE TAIL 18 steps 2841s; the first row 21:56:08, the last 08:34:54; THE SLOWEST OF 13b: T the gates chain, fourth pass (from the tape: ALL GREEN through the register gate) 709s; T the gates chain, fifth pass (--from checkpoint: checkpoint, stamp, sweep, unmoved — ALL GREEN) 580s; T the gates chain, first pass (tape, probes FAIL, daemon, dependency FAIL, build, journal, register; five skipped) 519s; T the gates chain, second pass (--from probes: the readers ran the world under the live name — refused; from the print, the file overwritten) 514s; T the positions table by FOUR workers outside the chain (the step's own command) 477s; B the fast checker on part 1 alone (ch15_fastcheck.py — the ink block, the clock, the scans, the callees' facts) 262s; A the docket scan (ch15_docket_scan.py — the link rows, the ranges sized, the prior reads credited) 245s; B the tape, first run 225s. The table itself: <scratch>/ch15b_timing.tsv, copied to the forms folder. The forms in World/step9/forms_deuteronomy_walk/.
