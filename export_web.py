#!/usr/bin/env python3
"""
export_web.py — SERVE-layer exporter: torah_grok.sqlite -> web/scroll/data/
...
OUT = ROOT / "web" / "scroll" / "data"

_spec = importlib.util.spec_from_file_location(
    "rdb", ROOT / "logic" / "pre_logic_methods_2026-07-28" /
    "render_flat_ledger_from_db.py")
...
def load_jps(shape):
    ...
    jps, skipped = {}, []
    for b in BOOKS:
        path = ROOT / "Data" / ("tanakh_%s_jps1917_en.json" % BOOK_NAMES[b].lower())
        if not path.exists():
            continue
...
def main():
    ...
    # logic/oral_audit/ledgers (the law-era receipts) is O3 for all its
    # verses — its rows anchor to the chapter's law, so per-verse
    # fractions there would undercount and are forbidden.
    ledger_ch = set()
    for f in (ROOT / "logic" / "oral_audit" / "ledgers").glob("*_*.jsonl"):
        bk, _, chs = f.stem.rpartition("_")
        if bk in BOOKS and chs.isdigit():
            ledger_ch.add((bk, int(chs)))

    # proven-by-cases: chapters compiled to a chapter machine in
    # logic/law_era (mirrored to the public repo as machines/<ch>/, where
    # the scenes gate guards the baseline's green)
    ABBR3 = {"gen": "Gen", "exo": "Exod", "lev": "Lev",
             "num": "Num", "deu": "Deut"}
    proven_ch = set()
    for f in (ROOT / "logic" / "law_era").glob("*_v2_DRAFT.py"):
        m = re.match(r"^([a-z]{3})_(\d+)_v2_DRAFT$", f.stem)
        if m and m.group(1) in ABBR3:
