...
## THE PUBLIC REPO (2026-09-15; the owner: "change the description", "I also want to make torahsim public", "eliminate this yes. also assign the most free license agreement")

THE CHECKS BEFORE THE FLIP: no secret ever committed (the backup-codes file: 0 commits), no key-like string in the tree or the history, the
usernames gone; the Bible text the Open Scriptures Hebrew Bible (Creative Commons Attribution 4.0); the 303 tracked texts and the snapshot
store's 212 works classical Hebrew works from Sefaria on their own terms; the owner's own address once in an old note (his, and on every
commit as the author anyway); THE ONE BLOCK: Data/reference_books — a copyrighted academic book (the Talmudic Logic series, a 4.8 MB PDF
and its text), read by no code.
THE BOOK PURGED: removed from the tree (7ce714d, the folder ignored from now on; a copy left in the session's scratch), then from every
commit by a second git filter-repo pass (--invert-paths, 0.7 s), main and the tag force-pushed again (the tag moved a second time, the
release keeping its asset); reviews/COMMIT_MAP_2026-09-15.txt rebuilt with three columns — the original id, the first rewrite's, the
second's (filter-repo keys both maps by the original id: 9e6c81a → 7c18242 → d52ad0f; the tip b4032b8 at the flip).
THE LICENSE: CC0 1.0 Universal, the public-domain dedication (the freest: no conditions), its legal text fetched from Creative Commons into
LICENSE; DATA_SOURCES.md at the root names what others wrote and on what terms (OSHB, JPS 1917, Sefaria, Strong's; the shelf as fetched).
TORAHCODE: archived, so unarchived, its note changed to "Retired in 2026. The work continues in TorahSim", archived again.
THE FLIP: `gh repo edit --visibility public` — Josephtorah/TorahSim PUBLIC; the release asset answers by its plain address (HTTP 200,
153,755,648 bytes), so a clone anywhere fetches the store with no login; then an anonymous clone into a temporary folder fetched it
through SETUP's own steps (the print in the state doc's addendum).
