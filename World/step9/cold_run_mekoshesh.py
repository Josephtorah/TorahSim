...
# not been declared what should be done to him" — the MODE the uncertainty, the liability standing since Exod
# 31:14), and the tent's output (15:35) is a bare sentence whose "die shall die" is the generations' rule — THE
# MODE installed INTO an existing law (the Exodus engine's cell sabbath('death_run'), which imported it from this
# span's run on 2026-09-07 before Numbers had a reading: measured, and given its home here). What this runner
# compiles is what no runner held: the forewarning that names the labor, the custody rule, the stoning protocol
# from the platform to the grave, the two verses reconciled, the hanging fork, the labor and the identity as
# data rows, and the case as one pipeline. Five motions of the deliverable rule, the wrap the sixth; every cell
# cites its source; every token probed (zero-report law); effects on every cell (the effects law). Reading
# ledger: logic/oral_triage/num_15_wood_tzitzit_2026-09-09.md (the Sifrei on Numbers 113-115 + Onkelos); the
# exam's docket: num_15_mekoshesh_exam_2026-09-09.md.
...
assert GUARDED == 34, ('the guard counted %d expectations, the tripwire holds 34' % GUARDED)   # measured by the guard on the first run (the hand count read 33)
...
PROBES = [
    ('Num', 'מקשש',   15, 32, 'gathering — the labor word'),
    ('Num', 'השבת',   15, 32, 'the Sabbath'),
    ('Num', 'המצאים', 15, 33, 'those who found him — the finders, the witnesses, the warners'),
    ('Num', 'מקשש',   15, 33, 'gathering — the repetition that teaches the warning'),
    ('Num', 'במשמר',  15, 34, 'in the guard — the halt'),
    ('Num', 'פרש',    15, 34, 'declared — what had not been'),
    ('Num', 'יומת',   15, 35, 'shall be put to death — the generations'),
    ('Num', 'רגום',   15, 35, 'stone — the hour'),
    ('Num', 'באבנים', 15, 35, 'with stones'),
    ('Num', 'העדה',   15, 35, 'the congregation — in its presence'),
    ('Num', 'למחנה',  15, 35, 'the camp — outside it'),
    ('Num', 'וירגמו', 15, 36, 'and they stoned'),
    ('Num', 'וימת',   15, 36, 'and he died'),
    ('Lev', 'אבן',    24, 23, 'a stone — the blasphemer\'s singular'),
    ('Exod', 'מחלליה', 31, 14, 'its profaners — the standing liability'),
    ('Deut', 'העדים', 17, 7,  'the witnesses — their hand first (OWED FORWARD to Deuteronomy)'),
    ('Deut', 'ותלית', 21, 22, 'and you shall hang (OWED FORWARD)'),
]
...
DATA = {
    'gatherers_labor': {
        'value': 'detaching',
        'settings': {
            'detaching': "he detached still-growing branches (תולש, 'detaching' — Sifrei Bamidbar 113:1's own word; the baraita at Shabbat 96b:15)",
            'carrying': "he carried four cubits in the public domain — Rav Yehuda in Shmuel's name (Shabbat 96b:15)",
            'gathering': "he gathered sticks into a pile (מעמר, 'binding sheaves') — Rav Acha son of Rabbi Yaakov (Shabbat 96b:15); Onkelos Num 15:32's verb, 'collecting'"},
        'source': "the ink says 'gathering wood' (מקשש עצים) and names no labor of the thirty-nine — every arm is capital (Shabbat 96b:18: 'with regard to that labor there is no uncertainty')"},
    ...
    'hanging_after_stoning': {
        'value': 'blasphemer_and_idolater',
        'settings': {
            'blasphemer_and_idolater': "only the blasphemer and the idolater are hanged after stoning — the sages (Mishnah Sanhedrin 6:4; Sanhedrin 45b)",
            'all_the_stoned': "all who are stoned are hanged — Rabbi Eliezer (Mishnah Sanhedrin 6:4); the Sifrei's own arm at 15:36 ('hang him, and they hanged him' — Deut 21:22)"},
        'source': "Deut 21:22 'if there be in a man a sin of a death sentence, and he be put to death, and you hang him on a tree' — the ink names no class; the Sifrei (114:1) reads the gatherer hanged, the Mishnah's sages exclude him"},
    'stoning_house_height': {
        'value': 'two_heights',
        'settings': {'two_heights': "the place of stoning was two men's heights (Mishnah Sanhedrin 6:4; Sifrei 114:1 'two stories'); Sanhedrin 45a: with his own height, three"},
        'source': "the ink says 'stone him' and no height — the platform is a DATUM"},
}
...
def narrative():
    ...
    g = 'the-wood-gatherer'
    return (n(g, 'labor_barred'), n(g, 'put_to_death'), n(g, 'stoned'), is_open(g, 'stoned'), n(g, 'warned_specifying_the_labor'), n(g, 'in_custody'), is_open(g, 'in_custody'), is_open(g, 'put_to_death'),
...
NARRATIVE_PREDICTED = (1, 1, 1, [False], 1, 1, [False], [False], 1, False, ['law_mekoshesh', 'law_sabbath'], 1, 'law_sabbath:death_run', 0)   # THE_TENT.md section 3 + THE FIRST RUN'S READING (the death sentence closed by the deed too — the eighth slot added after the ask-tool showed put_to_death OPEN): law_sabbath decides at the act (the liability and the mode), the warning on the person; the guard and the docket, covered by both daemons; the sentence installs the cell, no second stoned; the execution closes both body entries; no hanging on the sages' arm
...
CASES = [
    ...
    ('THE NARRATIVE on the world engine — the tape\'s four lines with the tent daemon first and the Sabbath engine second (the tripwire typed from THE_TENT.md section 3)',
     ...
     (1, 1, 1, [False], 1, 1, [False], [False], 1, False, ['law_mekoshesh', 'law_sabbath'], 1, 'law_sabbath:death_run', 0)),
]
