...
for u in num_28_daily_shabbat_rosh num_28_pesach_shavuot num_29_fall_festivals; do
  ...
  python3 "$ROOT"/logic/solo_tools/verify_text.py $u > $SP/offerings_vt_$u.out 2>&1; echo "exit $? $(tail -1 $SP/offerings_vt_$u.out)" >> $SP/offerings_chain.log
  echo "=== ritual $u $(date +%H:%M:%S)" >> $SP/offerings_chain.log
  python3 "$ROOT"/logic/solo_tools/freeze_ritual.py $u > $SP/offerings_ritual_$u.out 2>&1
