...
RANGE = re.compile(r'(Gen|Exod|Lev|Num)\s+(\d+):(\d+)(?:-(\d+))?')
...
mk('Exod 12:29', 'F', [], "w.marker('Exod 12:29', M['exodus'], value='at half of the night the LORD struck every firstborn — the night belongs to the fifteenth (the day boundary is the evening): the exodus day (12:41, 12:51)')", 'the night of the fifteenth')
...
for key, order, line, meta in items:
    ch = (key[0], key[1])
    if ch != last_ch:
        body.append('    # ---- %s %d ----' % ({1: 'Gen', 2: 'Exod', 3: 'Lev', 4: 'Num'}[key[0]], key[1])); last_ch = ch
    body.append(line)
