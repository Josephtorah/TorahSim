...
#!/usr/bin/env python3
# THE NUMBERS WALK 4b — THE CORPUS-WIDE DIFF: the parser 3b left (cold_run_sequence.pre_shelach.py's INK block) against the parser after
# the Shelach rules (the fraction, the unit noun, the definite one, the third generation, Sheshai, the tithe verb), over EVERY verse of the four books (and the marker verses of Deuteronomy the probes name) — numbers AND ordinals.
...
OLD = ink('<scratch>/cold_run_sequence.pre_shelach.py')
...
for b, c, v in verses:
    ...
    if no != nn or oo != on:
        moved.append((b, c, v, no, nn, oo, on, ' '.join(w for w in wn if w.rstrip('*^|~#').endswith(('שני', 'שתי', 'אחד', 'אחת', 'עשתי', 'יומים', 'אמתים', 'שלשת')) or w in NEW['UNITS'] or w.rstrip('|^') in NEW['UNITS'] or w == 'וחצי')))
