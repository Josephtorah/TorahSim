...
#!/usr/bin/env python3
# THE NUMBERS WALK, sitting 11 — MIDIAN, Numbers 31:1-54 (2026-09-12; the owner: "ok go" after the recovery-file rereads, on the ruling READ THEN
# COMPILE): THE FIRST MEASUREMENT PASS — the shelf's heads by position (the Sifrei's piskaot on 31), the draft's span (num_31_midian 31:1-54;
# the portion Matot's second chapter; the next draft num_32_gad_reuben
# opens at 32:1), the prior-read scan, the engine's parser on every verse of the chapter (the spoil's census, the fractions, the shekels — measured), and the DUMPS the
...
print('HEADS 154-162:', {p: heads.get(p) for p in range(154, 163)})
print('TOTAL piskaot', len(sif), 'last non-empty', max(p for p, r in enumerate(sif, 1) if r))
P = sorted(p for p, h in heads.items() if h and h[0] == 'Bamidbar' and h[1] == 31)
print('PISKAOT on 31:', P, 'rows', {p: (len(sif[p - 1]), len(sif_he[p - 1])) for p in P})
for p in P:
...
print('ONK_LEN 30-33:', {c: (len(onk[c - 1]), len(onk_he[c - 1])) for c in (30, 31, 32, 33)})
...
for uid in ('num_30_vows', 'num_31_midian', 'num_32_gad_reuben'):
...
print('VC 30-32:', {c: VC[c] for c in (30, 31, 32)})
# THE PRIOR READS
...
print('prior Onkelos Num 31 rows:', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos Num 31:\d', t, re.M)))
print('prior Sifrei rows 157-158:', sorted((f, sorted(set(re.findall(r'Sifrei (?:Bamidbar )?(15[78]):\d', t)))) for f, t in LED.items() if re.search(r'Sifrei (?:Bamidbar )?15[78]:\d', t)))
print('ledgers naming Num 31 at all:', sorted((f, len(re.findall(r'Num(?:bers)? 31:\d+', t))) for f, t in LED.items() if re.search(r'Num(?:bers)? 31:\d+', t)))
# THE PARSER on every verse of 31
...
rows = db.execute("SELECT v.chapter, v.verse, w.he, w.morph FROM words w JOIN verses v ON w.verse_id=v.id WHERE v.book='Num' AND v.chapter IN (31, 32) ORDER BY v.id, w.idx").fetchall()
by = {}
...
for v in range(1, VC[31] + 1):
    vw = CS.verse_words('Num', 31, v)
    marked = [t for t in vw if t[-1] in '#~^%@|*']
    print(f'  31:{v:<3} N {N("Num", 31, v)!s:<34} O {O("Num", 31, v)!s:<10} marked {marked}')
print('  32:1', N('Num', 32, 1), O('Num', 32, 1))
# THE REGISTER: narrative verbs (wayyiqtol, HC/V.w) per verse
print('---- THE REGISTER (verbs with the narrative form per verse)')
for v in range(1, VC[31] + 1):
    reg = [(x, m) for x, m, _ in by[(31, v)] if m and re.search(r'V.w', m)]
    print(f'  31:{v:<3} {reg}')
# THE STORE'S GLOSSES
...
for v in range(1, VC[31] + 1):
    he = ' '.join(x for x, _, _ in by[(31, v)])
    acc = ' '.join(f'{x}[{"/".join(accents(raw))}]' for x, _, raw in by[(31, v)])
    morph = ' '.join(f'{x}:{m}' for x, m, _ in by[(31, v)])
    gl = ' | '.join(f'{hp}={g}' for hp, g in SG.get((31, v), []))
    print(f'\n== Num 31:{v}\n  HE  {he}\n  ACC {acc}\n  MOR {morph}\n  GL  {gl}\n  ONK {clean(onk[30][v - 1])}\n  ARM {plain(clean(onk_he[30][v - 1]))}')
