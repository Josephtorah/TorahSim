...
for u in num_30_vows; do
  ...
  python3 <repo-old>/logic/solo_tools/verify_text.py $u > $SP/vows_vt_$u.out 2>&1; echo "exit $? $(tail -1 $SP/vows_vt_$u.out)" >> $SP/vows_chain.log
  echo "=== ritual $u $(date +%H:%M:%S)" >> $SP/vows_chain.log
  python3 <repo-old>/logic/solo_tools/freeze_ritual.py $u > $SP/vows_ritual_$u.out 2>&1
