...
def source_class(cs):
    m = VERSE.match(cs or '')
    if m:
        return 'torah3' if m.group(1) in ('Gen', 'Exod', 'Lev', 'Num') else 'torah_other'   # THE TENT sitting 2 (2026-09-09): Numbers joins the tape's books
    return 'case_row'
...
json.dump({'rec': REC, 'worlds': {str(k): v for k, v in _worlds.items()}}, open('<scratch>/seq_recording.json', 'w'), ensure_ascii=False, indent=0)
