...
for u in num_31_midian; do
  ...
  python3 "$ROOT"/logic/solo_tools/verify_text.py $u > $SP/midian_vt_$u.out 2>&1; echo "exit $? $(tail -1 $SP/midian_vt_$u.out)" >> $SP/midian_chain.log
  echo "=== ritual $u $(date +%H:%M:%S)" >> $SP/midian_chain.log
  python3 "$ROOT"/logic/solo_tools/freeze_ritual.py $u > $SP/midian_ritual_$u.out 2>&1
