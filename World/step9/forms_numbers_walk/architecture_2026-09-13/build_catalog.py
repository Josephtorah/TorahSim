#!/usr/bin/env python3
# FUNCTION_CATALOG.md REBUILT (2026-09-13; the standing order of #165, sitting C). The fourteen hand-written cards of 2026-09-05 are
# kept verbatim (their scores re-verified against catalog_facts.json, rerun today over all 57 runners); the forty-three runners
# without a card get one GENERATED from the instruments — catalog_facts.json (the score, fractions and ledger-ops lines the
...
# ---- the old cards, verbatim, keyed by runner ----
old_cards = {}
for m in re.finditer(r'^### \d+\. (cold_run_\w+)\.py — (.*?)\n(.*?)(?=^### \d+\. |^## |\Z)', old, re.M | re.S):
    old_cards[m.group(1)] = (m.group(2).strip(), m.group(3).rstrip() + '\n')
...
HELPERS = {'strip', 'verse_text', 'words', 'raw_words', 'ink', 'move', 'dat', 'hyp', 'out', 'seats', 'tok', 'lemma_seats', 'lemma_tokens', 'N', 'O',
           ...
           'scene', 'narrative', 'main', 'watches_day', 'hearing_due', 'first_verse', 'lint', 'run', 'grade', 'check', 'assert_seats'}
...
brought to the fourth book 2026-09-13. The fourteen cards of 2026-09-05
stand as written, their scores checked again today; the forty-three other
runners carry cards BUILT FROM THE INSTRUMENTS — `catalog_facts.json`
...
daemon's row). Nothing in a generated card was typed by hand. Verse
...
out.append('\n## The fourteen cards of 2026-09-05, checked again today\n\nEach card above written on 2026-09-05 was rerun on 2026-09-13; the score line as printed today:\n\n| Runner | Score line, 2026-09-13 |\n|---|---|\n')
for name, sl in reverified:
    out.append('| `%s` | %s |\n' % (name, sl or '(no matrix line printed)'))
out.append('\n' + tail)
text = ''.join(out)
JARGON = {'patach': 'the vowel point', 'sheva': 'the vowel point', 'qamats': 'the vowel point', 'qamatz': 'the vowel point', 'hiriq': 'the vowel point',
          ...
          'dagesh': 'the doubling dot', 'etnachta': 'the mid-verse pause', 'tevir': 'the accent', 'maqqef': 'the joining stroke', 'wayyiqtol': 'the narrative verb form'}
...
print('written', len(text.split('\n')), 'lines; cards', n, '; old kept', len(reverified))
