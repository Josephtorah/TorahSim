...
def main():
    ...
    def r6():
        ...
        t = RG.TALLIES
        assert t['receipts'].get('CLOSE') == 16 and sum(t['receipts'].values()) == 69, t['receipts']   # THE DEUTERONOMY WALK 1b (2026-09-15): CLOSE 15 -> 16 — Num 27:22's receipt turned CLOSE (the commission's debit closed by its run); Deut 1:3 ACT and 1:19 / 1:41 CHAPTER inside the same 69 (typed from the gate's print {'CHAPTER': 8, 'NONE': 18, 'ACT': 22, 'EVENT': 5, 'CLOSE': 16}); sitting 11b's note follows:   # THE NUMBERS WALK 11b (2026-09-12): CLOSE 11 -> 15 — the four receipts of chapter 31 (31:7, 31:31, 31:41, 31:47) turned CLOSE by the closes by value; typed from the gate's print; sitting 10b's note follows:   # THE NUMBERS WALK 10b (2026-09-12): the second form's eleven seats — typed from the first run's print after the finder: CLOSE 9 -> 11, ACT 18 -> 21, CHAPTER 5 -> 7, NONE 21 -> 25, EVENT 5
        assert sum(t['footers'].values()) == 9 and t['footers'].get('EMPTY') == 1, t['footers']   # THE DEUTERONOMY WALK 11b (2026-09-21): EMPTY 3 -> 1 — the header Deut 12:1 and the footer Deut 28:69 DAEMONS by law_seducers (given_at Deut 13:1 inside the one block they share); typed from the gate's print {'DAEMONS': 8, 'EMPTY': 1}; sitting 1b's note follows:   # THE DEUTERONOMY WALK 1b (2026-09-15): EMPTY 4 -> 3 — the footer Deut 4:45 DAEMONS by law_opening_speech (given_at Deut 1:16 inside its block); sitting 11b's note follows:   # THE NUMBERS WALK 11b (2026-09-12): EMPTY 5 -> 4 — the footer Num 36:13 DAEMONS by law_midian; typed from the gate's print
