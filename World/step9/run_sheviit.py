#!/usr/bin/env python3
"""run_sheviit.py — run round 47 (THE TOPIC-ROUTED DOCKET SUPPLEMENT — Mishnah Sheviit whole + the unread rows of Arakhin, Temurah, Bekhorot 9, Bava Metzia 5) against the engine.

Loads cases_sheviit.yaml, validates every input against the vocabulary
...
agreement. Same discipline as run_egypt.py."""
import sys
...
def main():
    spec = yaml.safe_load(open(HERE / "cases_sheviit.yaml",
                               encoding="utf-8"))
    ...
    print("\nSHEVIIT SUPPLEMENT: %d cases, %d answered, %d matched, %d misses"
