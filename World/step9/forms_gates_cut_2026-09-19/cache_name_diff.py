...
import sys, os, subprocess, json, io, contextlib, re, importlib
ROOT = _ROOT; sys.path.insert(0, os.path.join(ROOT, 'World', 'step9'))
with contextlib.redirect_stdout(io.StringIO()):
    import cold_run_sequence
def canon(v, depth=0):
    ...
    if isinstance(v, bytes): return repr(v)
...
pairs = [('cold_run_clocks', '_r'), ('cold_run_korach', 'V5_ASHAM'), ('cold_run_metzora', '_r'), ('cold_run_minchah', '_sin'), ('cold_run_mishpatim', 'TOTAL'), ('cold_run_mishpatim_2', 'TOTAL')]
out = {}
for m, n in pairs:
    v = getattr(sys.modules[m], n, '<absent>'); out[m + '.' + n] = repr(v)[:600]
