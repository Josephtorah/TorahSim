...
## THE GATES CUT — the design and the as-built (2026-09-19; the owner, after sitting 6b's chain: "What is gate chain and why does it take so long" →
...
AS RUN: two chains, ALL GREEN both — the first with everything in full, 43 min 23 s (wall 43 min 13 s); the second with nothing moved, 18 min 27 s (wall 17 min 49 s) — against
...
memory's cost rules, the state doc's #197, the addenda §45; the forms in World/step9/forms_gates_cut_2026-09-19/. LESSONS: MEASURE BEFORE CUTTING
...
TEST IT AGAINST A MOVED PIECE BEFORE TRUSTING IT (the regions form was struck by its own test).
  D38 THE VERIFIED-IMPORT CACHE (the same day, on "ok do it make it permanent" — GATES_CHAIN.md's D38 section the record) — after the cut the
     ...
     aside, the sharing groups the same, the engine's own modules the same) 8/8 after seventeen slips each made a rule. MEASURED: the load 169 s
     → 35 s; the tape step 155 → 27 s; the checkpoint suite 205 → 75 s; the chain 16 min 23 s (the sweep full: a shared piece moved).
