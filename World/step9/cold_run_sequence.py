...
# registers. Every import is a live edge beyond the type census (the dependency gate lists such calls; no rule crosses).
...
# ALL 38 DAEMONS in registration order: the library's five first (the runners register them first on their own
...
DAEMON_ORDER = [
    ...
    ('cold_run_pre_sinai', 'law_pre_sinai'), ('cold_run_family', 'law_family'), ('cold_run_pesach', 'law_pesach'),
    ('cold_run_decalogue', 'law_decalogue'), ('cold_run_ordinances', 'law_ordinances'), ('cold_run_mishpatim', 'law_mishpatim'),
    ('cold_run_mishpatim_2', 'law_mishpatim_2'), ('cold_run_calendar', 'law_calendar'), ('cold_run_erection', 'law_erection'),
...
def tape(w, M, P):
    """THE STITCHED TAPE — generated by scratchpad/seq_stitch.py: 69 markers, 140 history events, 8 closes in canonical verse order"""
    ...
    # ---- Exod 7 ----
    assert_ink('Exod 7:7', [80, 83]); M['exodus_year'] = yr(w, w.clock.calendar.add(M['descent'], 430, 'year')) if P['sojourn_start']['value'] == 'descent_literal' else yr(w, w.clock.calendar.add(M['born:isaac'], 400, 'year')); M['exodus'] = cal_day(w, M['exodus_year'], 1, 15); M['born:moses'] = w.clock.calendar.add(M['exodus'], -80, 'year'); M['born:aaron'] = w.clock.calendar.add(M['exodus'], -83, 'year'); w.marker('Exod 7:7', M['born:moses'], value='Moses a son of eighty, Aaron eighty-three, at the speaking — the eras set from the exodus day', era='life:moses', proleptic=True); w.clock.set_era('life:aaron', M['born:aaron'], None)   # the life eras of Moses and Aaron (the speaking undated within the year)
    # ---- Exod 12 ----
...
def c3(w, M, R):
    ...
    cp('C3c the lifespans bound 133+137+80 (Exod 6:18, 6:20, 7:7)', None, yE - yD, bound=[0, 350])
    return yE
...
def run():
    ...
    closes_done = sum(1 for ent in w.entities.values() for e in ent.ledger if str(e.get('closed_by', '')).startswith('Gen '))
