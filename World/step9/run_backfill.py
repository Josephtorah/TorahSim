#!/usr/bin/env python3
"""run_backfill.py — run the Exodus 1-21 backfill exam's case rows against the engine.

Loads cases_exodus_backfill.yaml, validates every input against the vocabulary
...
agreement. Same discipline as run_sweep.py; the block's rules are the
first TALMUD-ONLY modules (no Mishnah row above them).
"""
import sys
...
def main():
    spec = yaml.safe_load(open(HERE / "cases_exodus_backfill.yaml",
                               encoding="utf-8"))
    ...
    for mod in modules:
        ...
        for case in mod.get("cases", []):
            ...
            if bad:
                print("   %-24s VOCABULARY REJECT: %s" % (cid, bad))
                misses.append((cid, "vocabulary", bad))
                continue
            out = engine.answer(name, inp)
            if not out:
                print("   %-24s ENGINE SILENT (expected: %s)"
            ...
            if ok:
                ...
                misses.append((cid, "mismatch", {"expected": exp,
                                                 "got": got}))
            print("   %-24s %s expected=%-28s got=%s"
                  % (cid, flag, exp[:28], ", ".join(got)))
    print("\nBACKFILL: %d cases, %d answered, %d matched, %d misses"
