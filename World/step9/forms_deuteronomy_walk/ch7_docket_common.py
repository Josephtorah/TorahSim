# THE DEUTERONOMY WALK 5b — the docket parts' shared helper: the dump's addresses in order (the source of truth for coverage — a part can never invent
...
# 'prefix:n' for lo <= n <= hi; a bare string matches one address. The notes name the cell of cold_run_seven_nations.py (F1 the_seven_nations, F2 the_holy_people,
# F3 the_faithful_god, F4 because_you_hear, F5 do_not_fear, F6 the_images_and_the_devoted) on LAW rows, or the callee's cell by CALL.
import os, re
SCR = os.path.dirname(os.path.abspath(__file__))
_dump = open(f'{SCR}/ch7_docket_dump.txt', encoding='utf-8').read()
