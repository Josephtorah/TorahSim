...
#!/usr/bin/env python3
# assemble cold_run_festivals_judges.py from the parts (the CASES literal from part 5 when it exists, else an empty table for the generator's pass). ch15_assemble.py's form. RUN FROM THE REPO ROOT.
import os, sys, subprocess, py_compile
ROOT = _ROOT
SP = os.path.dirname(os.path.abspath(__file__))
parts = [open(f'{SP}/ch16_part{n}.py', encoding='utf-8').read() for n in (1, 2, 3)]
p5 = f'{SP}/ch16_part5.py'
...
out = f'{ROOT}/World/step9/cold_run_festivals_judges.py'
