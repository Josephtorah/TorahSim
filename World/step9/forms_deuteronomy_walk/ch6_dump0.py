...
en_forms = re.compile(r'\((?:Deut(?:eronomy)?\.?|Dt\.?|Devarim) ?(6):(\d+)')
...
with open(f'{SP}/ch6_sifrei_outside.txt', 'w', encoding='utf-8') as f:
    for p, r in allrows:
        f.write(f'\n## Sifrei {p}:{r} (head {heads[p]}) HE\n{clean(he[p-1][r-1])}\n\n## Sifrei {p}:{r} EN\n{clean(en[p-1][r-1]) if r <= len(en[p-1]) else "(no EN row)"}\n')
print('  outside rows file bytes', os.path.getsize(f'{SP}/ch6_sifrei_outside.txt'))
...
with open(f'{SP}/ch6_onkelos.txt', 'w', encoding='utf-8') as f:
...
print('  onkelos dump bytes', os.path.getsize(f'{SP}/ch6_onkelos.txt'))
...
for v in sorted(set(hit) | {int(k.split(':')[1]) for k in numv} | {1, 4, 20, 25}):
...
print('  ledgers with an Onkelos Deut 6 row:', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos Deut 6:', t, re.M)))
...
print('  ledgers NAMING a verse of Deut 6:', len(NAMING)); [print('   ', n) for n in NAMING]
...
print('==== G. THE DRAFTS')
for uid in ('deu_05_decalogue', 'deu_06_shema', 'deu_07_nations_cherem'):
    ...
    if uid == 'deu_06_shema': print('    the step verses:', [b for a, b in st], '| covers DB', VC[CH], '?', [b for a, b in st] == list(range(1, VC[CH] + 1)))
print('  the units\' claim prefixes present anywhere?', {p: sum(1 for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/*_claims.json') + glob.glob(f'{ROOT}/logic/units/*.yaml') if f'{p}-' in open(f, encoding='utf-8').read()) for p in ('DV06A', 'DV05B', 'DV05', 'DV04B')})
print('  existing manifests deu_06*:', sorted(os.path.basename(f) for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/deu_06*')), '| ledgers deu_*:', sorted(f for f in LED if f.startswith('deu_')))
...
with open(f'{SP}/ch6_store_glosses.txt', 'w', encoding='utf-8') as f:
    for (c, v) in sorted(SG): f.write(f'{c}:{v}: ' + str([(i, t, g) for i, t, g in SG[(c, v)]]) + '\n')
print('  gloss file bytes', os.path.getsize(f'{SP}/ch6_store_glosses.txt'))
...
print('==== I. THE SPINE ON THE CHAPTER — the Sifrei piskaot whose head is in chapter 6, whole (the first on-chapter piskaot since chapter 3)')
SPINE = [31, 32, 33, 34, 35, 36]   # typed from A's print: the heads whose chapter is 6
rows = {p: (len(he[p - 1]), len(en[p - 1])) for p in SPINE}
print('  rows per piska (HE, EN):', rows, '| total HE rows', sum(a for a, _ in rows.values()), '| HE == EN everywhere?', all(a == b for a, b in rows.values()))
with open(f'{SP}/ch6_sifrei_spine.txt', 'w', encoding='utf-8') as f:
    for p in SPINE:
        for r in range(len(he[p - 1])): f.write(f'--- {p}:{r + 1}\nHE: {clean(he[p - 1][r])}\nEN: {clean(en[p - 1][r])}\n')
print('  spine file bytes', os.path.getsize(f'{SP}/ch6_sifrei_spine.txt'), '| bytes per piska HE+EN:', {p: sum(len(clean(he[p - 1][r])) + len(clean(en[p - 1][r])) for r in range(len(he[p - 1]))) for p in SPINE})
