...
print('  DB verses', len(D), '| export verses HE', len(E), 'EN', len(onk[CH - 1]), '| per-chapter export lengths vs DB, chapters 1-14:', [(c, len(onk_he[c - 1]), VC[c]) for c in range(1, 15) if len(onk_he[c - 1]) != VC[c]])
INF = 10 ** 9
...
print('  heads whose chapter is', CH, ':', [(p, h) for p, h in heads.items() if h and h[0] == CH], '| heads by chapter 1-14:', sorted(Counter(h[0] for h in heads.values() if h).items())[:14])
...
en_forms = re.compile(r'\((?:Deut(?:eronomy)?\.?|Dt\.?|Devarim) ?(14):(\d+)')
...
ibid = [(p, r+1, m.group(0), clean(row)[max(0, m.start()-120):m.start()]) for p in range(1, len(en)+1) for r, row in enumerate(en[p-1]) for m in re.finditer(r'\((?:Ibid|ibid)\.? ?14:(\d+)\)', clean(row))]
print('  EN "ibid 14:n" candidates:', len(ibid)); [print('    ', p, r, f, '<<', ctx[-100:].replace('\n', ' ')) for p, r, f, ctx in ibid[:40]]
...
with open(f'{SP}/ch14_sifrei_outside.txt', 'w', encoding='utf-8') as f:
    for p, r in OUTSIDE:
        f.write(f'\n## Sifrei {p}:{r} (head {heads[p]}) HE\n{clean(he[p-1][r-1])}\n\n## Sifrei {p}:{r} EN\n{clean(en[p-1][r-1]) if r <= len(en[p-1]) else "(no EN row)"}\n')
print('  outside rows file bytes', os.path.getsize(f'{SP}/ch14_sifrei_outside.txt'))
...
with open(f'{SP}/ch14_onkelos.txt', 'w', encoding='utf-8') as f:
...
print('  onkelos dump bytes', os.path.getsize(f'{SP}/ch14_onkelos.txt'))
...
for v in sorted(set(hit) | {int(k.split(':')[1]) for k in numv} | {1, 29}):
...
print('  ledgers with an Onkelos Deut 14 row:', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos Deut 14:', t, re.M)))
...
print('  ledgers NAMING a verse of Deut 14:', len(NAMING)); [print('   ', n) for n in NAMING]
...
print('==== G. THE DRAFTS')
for uid in sorted(os.path.basename(f)[:-5] for f in glob.glob(f'{ROOT}/logic/units/deu_13*.yaml') + glob.glob(f'{ROOT}/logic/units/deu_14*.yaml')):
    ...
    if uid.startswith('deu_14'): print('    the step verses:', [b for a, b in st], '| covers DB', VC[CH], '?', [b for a, b in st] == list(range(1, VC[CH] + 1)))
print('  the units\' claim prefixes present anywhere?', {p: sum(1 for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/*_claims.json') + glob.glob(f'{ROOT}/logic/units/*.yaml') if f'{p}-' in open(f, encoding='utf-8').read()) for p in ('DV14', 'DV14A', 'DV14B', 'DV13', 'DV15')})
print('  existing manifests deu_14*:', sorted(os.path.basename(f) for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/deu_14*')), '| ledgers deu_*:', sorted(f for f in LED if f.startswith('deu_')))
...
with open(f'{SP}/ch14_store_glosses.txt', 'w', encoding='utf-8') as f:
    for (c, v) in sorted(SG): f.write(f'{c}:{v}: ' + str([(i, t, g) for i, t, g in SG[(c, v)]]) + '\n')
print('  gloss file bytes', os.path.getsize(f'{SP}/ch14_store_glosses.txt'))
...
with open(f'{SP}/ch14_sifrei_spine.txt', 'w', encoding='utf-8') as f:
    for p in SPINE:
        for r in range(len(he[p - 1])): f.write(f'--- {p}:{r + 1} (head {heads[p]})\nHE: {clean(he[p - 1][r])}\nEN: {clean(en[p - 1][r]) if r < len(en[p - 1]) else "(no EN row)"}\n')
print('  spine file bytes', os.path.getsize(f'{SP}/ch14_sifrei_spine.txt'), '| bytes per piska HE+EN:', {p: sum(len(clean(he[p - 1][r])) + (len(clean(en[p - 1][r])) if r < len(en[p - 1]) else 0) for r in range(len(he[p - 1]))) for p in SPINE})
