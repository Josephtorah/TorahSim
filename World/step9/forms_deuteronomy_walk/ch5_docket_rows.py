#!/usr/bin/env python3
# print the docket dump's rows [a, b) with each text cut to N chars — the reading's instrument (one chunk per verdict part)
import sys, re, os
SCR = os.path.dirname(os.path.abspath(__file__))
a, b, N = int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3])
t = open(f'{SCR}/ch5_docket_dump.txt', encoding='utf-8').read()
blocks = t.split('\n## ')[1:]
