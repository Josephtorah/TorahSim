...
print('  DB verses', len(D), '| export verses HE', len(E), 'EN', len(onk[CH - 1]), '| per-chapter export lengths vs DB, chapters 1-16:', [(c, len(onk_he[c - 1]), VC[c]) for c in range(1, 17) if len(onk_he[c - 1]) != VC[c]])
INF = 10 ** 9
...
print('  heads 124-150 (HE first citation | rows | EN first quote):')
...
print('  heads whose chapter is', CH, ':', [(p, h) for p, h in heads.items() if h and h[0] == CH], '| heads by chapter 1-16:', sorted(Counter(h[0] for h in heads.values() if h).items())[:16])
...
en_forms = re.compile(r'\((?:Deut(?:eronomy)?\.?|Dt\.?|Devarim) ?(16):(\d+)')
...
ibid = [(p, r+1, m.group(0), clean(row)[max(0, m.start()-120):m.start()]) for p in range(1, len(en)+1) for r, row in enumerate(en[p-1]) for m in re.finditer(r'\((?:Ibid|ibid)\.? ?16:(\d+)\)', clean(row))]
print('  EN "ibid 16:n" candidates:', len(ibid)); [print('    ', p, r, f, '<<', ctx[-100:].replace('\n', ' ')) for p, r, f, ctx in ibid[:40]]
...
with open(f'{SP}/ch16_sifrei_outside.txt', 'w', encoding='utf-8') as f:
    for p, r in OUTSIDE:
        f.write(f'\n## Sifrei {p}:{r} (head {heads[p]}) HE\n{clean(he[p-1][r-1])}\n\n## Sifrei {p}:{r} EN\n{clean(en[p-1][r-1]) if r <= len(en[p-1]) else "(no EN row)"}\n')
print('  outside rows file bytes', os.path.getsize(f'{SP}/ch16_sifrei_outside.txt'))
...
with open(f'{SP}/ch16_onkelos.txt', 'w', encoding='utf-8') as f:
...
print('  onkelos dump bytes', os.path.getsize(f'{SP}/ch16_onkelos.txt'))
...
print('  dispositions naming Deut 16:', re.findall(r'^([^\n]*Deut 16:[^\n]*)$', RD, re.M)[:20])
RI = open(f'{ROOT}/World/step9/REGISTER_INDEX.md', encoding='utf-8').read()
print('  REGISTER_INDEX lines on Deut 16:'); [print('   ', l[:220]) for l in RI.split('\n') if re.search(r'Deut 16:', l)]
...
print('  ledgers with an Onkelos Deut 16 row:', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos Deut 16:', t, re.M)))
...
print('  ledgers NAMING a verse of Deut 16:', len(NAMING)); [print('   ', n) for n in NAMING]
print('  ledgers with an Onkelos Exod 12 / Exod 13 / Exod 23 / Exod 34 / Lev 23 / Num 9 / Num 28 / Num 29 row (the kin\'s reading — the Passover 12:1-28, 12:43-50, the unleavened bread and the month of Aviv 13:3-10, the three pilgrimages 23:14-19 and 34:18-26, the appointed times Leviticus 23, the second Passover Numbers 9, the offerings 28-29 for 16:1-17):', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos (?:Exod (?:12|13|23|34)|Lev 23|Num (?:9|28|29)):', t, re.M)), '| ledgers with an Onkelos Exod 18 / Exod 23 / Lev 19 / Deut 1 row (the judges and the bribe — Exodus 18:13-26, 23:1-8, Leviticus 19:15, Deuteronomy 1:9-18 for 16:18-20):', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos (?:Exod (?:18|23)|Lev 19|Deut 1):', t, re.M)), '| ledgers with an Onkelos Exod 34 / Lev 26 / Deut 7 / Deut 12 row (the asherah and the pillar — Exodus 34:13, Leviticus 26:1, Deuteronomy 7:5, 12:3 for 16:21-22):', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos (?:Exod 34|Lev 26|Deut (?:7|12)):', t, re.M)))
print('==== G. THE DRAFTS')
for uid in sorted(os.path.basename(f)[:-5] for f in glob.glob(f'{ROOT}/logic/units/deu_15*.yaml') + glob.glob(f'{ROOT}/logic/units/deu_16*.yaml')):
    ...
    if uid.startswith('deu_16'): print('    the step verses:', [b for a, b in st], '| covers DB', VC[CH], '?', [b for a, b in st] == list(range(1, VC[CH] + 1)))
print('  the units\' claim prefixes present anywhere?', {p: sum(1 for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/*_claims.json') + glob.glob(f'{ROOT}/logic/units/*.yaml') if f'{p}-' in open(f, encoding='utf-8').read()) for p in ('DV16', 'DV16A', 'DV16B', 'DV15', 'DV17')})
print('  existing manifests deu_16*:', sorted(os.path.basename(f) for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/deu_16*')), '| ledgers deu_*:', sorted(f for f in LED if f.startswith('deu_')))
...
with open(f'{SP}/ch16_store_glosses.txt', 'w', encoding='utf-8') as f:
    for (c, v) in sorted(SG): f.write(f'{c}:{v}: ' + str([(i, t, g) for i, t, g in SG[(c, v)]]) + '\n')
print('  gloss file bytes', os.path.getsize(f'{SP}/ch16_store_glosses.txt'))
...
print('==== I. THE SPINE ON THE CHAPTER — the Sifrei piskaot whose head is in chapter', CH, '(the spine on the chapter an eighth time), whole')
...
with open(f'{SP}/ch16_sifrei_spine.txt', 'w', encoding='utf-8') as f:
    for p in SPINE:
        for r in range(len(he[p - 1])): f.write(f'--- {p}:{r + 1} (head {heads[p]})\nHE: {clean(he[p - 1][r])}\nEN: {clean(en[p - 1][r]) if r < len(en[p - 1]) else "(no EN row)"}\n')
print('  spine file bytes', os.path.getsize(f'{SP}/ch16_sifrei_spine.txt'), '| bytes per piska HE+EN:', {p: sum(len(clean(he[p - 1][r])) + (len(clean(en[p - 1][r])) if r < len(en[p - 1]) else 0) for r in range(len(he[p - 1]))) for p in SPINE})
...
print('  THE PORTION EDGE inside the chapter (Re\'eh ends at 16:17, Shoftim opens at 16:18) — the heads by verse, and the verses of the chapter with no head:', sorted({heads[p][1] for p in SPINE}), [v for v in range(1, VC[CH] + 1) if v not in {heads[p][1] for p in SPINE}])
