...
#!/usr/bin/env python3
# THE DEUTERONOMY WALK sitting 12 — CHAPTER 14 (2026-09-21): the store's glosses at the chapter's seats read back into the display layer's override file — BY
# GLOSS where the store's every token of the gloss is the one word (the census in ch14_ink.py, GLOSS_FAMILY asserted against the store) and BY REFERENCE
# where the family is mixed. Display only; the draft unit untouched. THE ROWS ARE READ FROM ch14_ink.py's OWN LISTS. Run ONCE (the anchors assert the rows
# absent first). Sitting 11's form (ch13_patch_overrides.py): the anchors the LAST ROWS of sitting 11's two blocks — the by_gloss block's last row found by
...
assert len(GL) == 49 and len(REF) == 131, (len(GL), len(REF))
P = f'{ROOT}/logic/glosses/word_gloss_overrides.yaml'
t = open(P, encoding='utf-8').read()
assert '"Deut.14.' not in t and all(f'"{k}": ' not in t for k, _ in GL), [k for k, _ in GL if f'"{k}": ' in t]
MARK = 'THE DEUTERONOMY WALK sitting 12 (2026-09-21, Deuteronomy 14)'
assert MARK not in t
M5 = 'THE DEUTERONOMY WALK sitting 11 (2026-09-21, Deuteronomy 13)'
...
assert len(rows) == 34, len(rows)   # sitting 11's thirty-four by-gloss rows
A1 = rows[-1]; assert t.count(A1) == 1
gl_block = A1 + f'\n  # {MARK}: the store\'s glosses whose every token is the one word (censused in the reading\'s ink script, ch14_ink.GLOSS_FAMILY) — RESEARCH_LOG.md\n' + ''.join(f'  "{k}": "{v}"\n' for k, v in GL)
