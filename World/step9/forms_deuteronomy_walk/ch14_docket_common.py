# THE DEUTERONOMY WALK 12b (11b's form by derive_ch14_docket_tools.py) — the docket parts' shared helper: the dump's addresses in order (the source of truth for coverage — a part can never invent
...
# 'prefix:n' for lo <= n <= hi; a bare string matches one address. The notes name the cell of cold_run_food_tithe.py (F1 the_sons_and_the_cuttings, F2 the_beasts, F3 the_water_and_the_birds, F4 the_carcass_and_the_kid,
# F5 the_second_tithe, F6 the_far_place_the_money_the_rejoicing_and_the_levite, F7 the_third_year — the design's seven cells on the seven claims' spans; the_readback the table) on LAW rows, or the callee's cell by CALL.
import os, re
SCR = os.path.dirname(os.path.abspath(__file__))
_dump = open(f'{SCR}/ch14_docket_dump.txt', encoding='utf-8').read()
...
from ch14_credited_rows import CREDITED_SPEC   # 12b: the credited rows carried with their ledgers (ch14_credit_carry.py — 11b's form); a part prepends it to its SPEC
