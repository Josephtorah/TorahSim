# THE DEUTERONOMY WALK 7b — the docket parts' shared helper: the dump's addresses in order (the source of truth for coverage — a part can never invent
...
# 'prefix:n' for lo <= n <= hi; a bare string matches one address. The notes name the cell of cold_run_not_righteousness.py (F1 the_frame, F2 the_calf_retold, F3 the_forty_days, F4 aarons_peril, F5 the_four_provocations,
# F6 the_intercession) on LAW rows, or the callee's cell by CALL.
import os, re
SCR = os.path.dirname(os.path.abspath(__file__))
_dump = open(f'{SCR}/ch9_docket_dump.txt', encoding='utf-8').read()
