...
en_forms = re.compile(r'\((?:Deut(?:eronomy)?\.?|Dt\.?|Devarim) ?(7):(\d+)')
...
ibid = [(p, r+1, m.group(0), clean(row)[max(0, m.start()-120):m.start()]) for p in range(1, len(en)+1) for r, row in enumerate(en[p-1]) for m in re.finditer(r'\((?:Ibid|ibid)\.? ?7:(\d+)\)', clean(row))]
print('  EN "ibid 7:n" candidates:', len(ibid)); [print('    ', p, r, f, '<<', ctx[-100:].replace('\n', ' ')) for p, r, f, ctx in ibid[:40]]
...
with open(f'{SP}/ch7_sifrei_outside.txt', 'w', encoding='utf-8') as f:
    for p, r in allrows:
        f.write(f'\n## Sifrei {p}:{r} (head {heads[p]}) HE\n{clean(he[p-1][r-1])}\n\n## Sifrei {p}:{r} EN\n{clean(en[p-1][r-1]) if r <= len(en[p-1]) else "(no EN row)"}\n')
print('  outside rows file bytes', os.path.getsize(f'{SP}/ch7_sifrei_outside.txt'))
...
with open(f'{SP}/ch7_onkelos.txt', 'w', encoding='utf-8') as f:
...
print('  onkelos dump bytes', os.path.getsize(f'{SP}/ch7_onkelos.txt'))
...
for v in sorted(set(hit) | {int(k.split(':')[1]) for k in numv} | {1, 26}):
...
print('  ledgers with an Onkelos Deut 7 row:', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos Deut 7:', t, re.M)))
...
print('  ledgers NAMING a verse of Deut 7:', len(NAMING)); [print('   ', n) for n in NAMING]
...
print('==== G. THE DRAFTS')
for uid in sorted(os.path.basename(f)[:-5] for f in glob.glob(f'{ROOT}/logic/units/deu_0[6-8]*.yaml')):
    ...
    if uid.startswith('deu_07'): print('    the step verses:', [b for a, b in st], '| covers DB', VC[CH], '?', [b for a, b in st] == list(range(1, VC[CH] + 1)))
print('  the units\' claim prefixes present anywhere?', {p: sum(1 for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/*_claims.json') + glob.glob(f'{ROOT}/logic/units/*.yaml') if f'{p}-' in open(f, encoding='utf-8').read()) for p in ('DV07', 'DV07A', 'DV07B', 'DV06', 'DV08')})
print('  existing manifests deu_07*:', sorted(os.path.basename(f) for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/deu_07*')), '| ledgers deu_*:', sorted(f for f in LED if f.startswith('deu_')))
...
with open(f'{SP}/ch7_store_glosses.txt', 'w', encoding='utf-8') as f:
    for (c, v) in sorted(SG): f.write(f'{c}:{v}: ' + str([(i, t, g) for i, t, g in SG[(c, v)]]) + '\n')
print('  gloss file bytes', os.path.getsize(f'{SP}/ch7_store_glosses.txt'))
