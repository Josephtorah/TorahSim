#!/usr/bin/env python3
# THE DEUTERONOMY WALK 12b (2026-09-22) — RUN B's and THE TAIL's instruments and prints copied into World/step9/forms_deuteronomy_walk/ as the walk's forms — the
...
# recorder and the stitcher copied under their sitting's names; the chain's folder whole. (RUN A's forms were copied at the docket run — copy_ch14_docket_forms.py
# and the design's copier.) copy_ch13b_forms.py's form. RUN FROM THE REPO ROOT.
...
PY = ['derive_ch14_recon.py', 'ch14_compile_recon.py', 'derive_ch14_docket_scan.py', 'ch14_docket_scan.py', 'write_ch14b_design.py', 'write_ch14b_cleanpoint.py',   # RUN A (the absent ones skipped by name)
      'patch_probes_ch14.py', 'add_types_ch14.py', 'ch14_callees.py', 'ch14_callees_facts.py', 'derive_ch14_part1.py', 'ch14_part1.py', 'ch14_part2.py', 'ch14_part3a.py', 'ch14_part3b.py', 'ch14_part3c.py', 'ch14_part3.py',
      'ch14_part4a.py', 'ch14_part4b.py', 'ch14_part4c.py', 'ch14_part4.py', 'ch14_part5.py', 'ch14_scene_gen.py', 'ch14_cases_gen.py', 'ch14_assemble.py', 'ch14_fastcheck.py', 'derive_ch14_shells.py',
      'derive_ch14_seq_tools.py', 'seq_record_ch14.py', 'seq_stitch_ch14.py', 'patch_seq_literals_ch14.py', 'write_ch14b_runb_point.py', 'write_ch14b_records.py', 'copy_ch14b_forms.py', 'extend_commit_msg_ch14b.py',
      'ch14_runner_chain.sh', 'ch14_tape_chain.sh', 'patch_tail_ch14b.py', 'tstep.sh']   # RUN B and the tail
OUT = ['ch14_recon.out', 'ch14_recon.err', 'ch14_docket_scan.out',   # RUN A's prints
       'ch14b_probes_fail.out', 'add_types_ch14.out', 'ch14_callees.out', 'ch14_fastcheck_run0.out', 'ch14_fastcheck_run1.out', 'ch14_fastcheck_run2.out', 'ch14_fastcheck_run3.out', 'ch14_cases_gen.out', 'ch14_runner_run1.out', 'ch14_runner_run2.out',
       'ch14_runner_chain.log', 'seq_record_ch14.out', 'seq_stitch_ch14.out', 'patch_seq_literals_ch14.out', 'ch14_tape_run1.out', 'ch14_tape_run2.out', 'ch14_checkpoint_check.out', 'ch14_tape_chain.log',
       'ch14b_chain1.log', 'ch14b_chain2.log', 'ch14b_chain3.log', 'ch14b_chain_SUMMARY_first.txt', 'ch14b_chain_SUMMARY_second.txt', 'ch14b_dependency_direct.out', 'patch_tail_ch14b.out', 'ch14b_positions_direct.log',
       'ch14b_chain_story.txt', 'ch14b_tape_story.txt', 'ch14b_records_check.out', 'ch14b_records_write.out', 'commit_msg_ch14b.txt', 'ch14b_timing.tsv']
GATES = 'gates_ch14b'   # the chain's folder — every step's print and the SUMMARY
...
if os.path.isdir(g):
    for f in sorted(os.listdir(g)):
        ...
        open(f'{DST}/gates_ch14b_{f}', 'w', encoding='utf-8').write(t); n += 1
