#!/usr/bin/env python3
# DEUTERONOMY CHAPTER 5, THE READING (THE DEUTERONOMY WALK sitting 3, 2026-09-16; the owner: "go" after the rereads) — THE FIRST MEASUREMENT
# PASS, nothing typed: (A0) THE TWO DIVISIONS — the export's chapter 5 (thirty verses) aligned to the DB's (thirty-three) by a monotone
...
print('  DB verses', len(D), '| export verses HE', len(E), 'EN', len(onk[CH - 1]), '| per-chapter export lengths vs DB, chapters 1-8:', [(c, len(onk_he[c - 1]), VC[c]) for c in range(1, 9) if len(onk_he[c - 1]) != VC[c]])
INF = 10 ** 9
...
print('  heads whose chapter is', CH, ':', [(p, h) for p, h in heads.items() if h and h[0] == CH], '| heads by chapter 1-8:', sorted(Counter(h[0] for h in heads.values() if h).items())[:8])
...
en_forms = re.compile(r'\((?:Deut(?:eronomy)?\.?|Dt\.?|Devarim) ?(5):(\d+)')
...
ibid = [(p, r+1, m.group(0), clean(row)[max(0, m.start()-120):m.start()]) for p in range(1, len(en)+1) for r, row in enumerate(en[p-1]) for m in re.finditer(r'\((?:Ibid|ibid)\.? ?5:(\d+)\)', clean(row))]
print('  EN "ibid 5:n" candidates:', len(ibid)); [print('    ', p, r, f, '<<', ctx[-100:].replace('\n', ' ')) for p, r, f, ctx in ibid[:40]]
...
with open(f'{SP}/ch5_sifrei_outside.txt', 'w', encoding='utf-8') as f:
    for p, r in allrows:
        f.write(f'\n## Sifrei {p}:{r} (head {heads[p]}) HE\n{clean(he[p-1][r-1])}\n\n## Sifrei {p}:{r} EN\n{clean(en[p-1][r-1]) if r <= len(en[p-1]) else "(no EN row)"}\n')
print('  outside rows file bytes', os.path.getsize(f'{SP}/ch5_sifrei_outside.txt'))
...
with open(f'{SP}/ch5_onkelos.txt', 'w', encoding='utf-8') as f:
...
print('  onkelos dump bytes', os.path.getsize(f'{SP}/ch5_onkelos.txt'))
...
for v in sorted(set(hit) | {int(k.split(':')[1]) for k in numv} | {1, 22, 31}):
...
print('  dispositions naming Deut 5:', re.findall(r'^([^\n]*Deut 5:[^\n]*)$', RD, re.M)[:20])
RI = open(f'{ROOT}/World/step9/REGISTER_INDEX.md', encoding='utf-8').read()
print('  REGISTER_INDEX lines on Deut 5:'); [print('   ', l[:220]) for l in RI.split('\n') if re.search(r'Deut 5:', l)]
...
print('  ledgers with an Onkelos Deut 5 row:', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos Deut 5:', t, re.M)))
...
NAMING = sorted((f, len(re.findall(r'Deut(?:eronomy)? 5:\d+', t)), sorted(set(re.findall(r'Deut(?:eronomy)? 5:\d+(?:-\d+)?', t)))[:14]) for f, t in LED.items() if re.search(r'Deut(?:eronomy)? 5:\d+', t))
print('  ledgers NAMING a verse of Deut 5:', len(NAMING)); [print('   ', n) for n in NAMING]
print('  ledgers with an Onkelos Exod 20 row (the first copy\'s reading):', sorted(f for f, t in LED.items() if re.search(r'^- Onkelos Exod 20:', t, re.M)), '| Mekhilta rows on Exod 20 anywhere:', sum(len(re.findall(r'Mekhilta[^\n]{0,40}20:\d+', t)) for t in LED.values()))
print('==== G. THE DRAFTS')
for uid in ('deu_04_refuge_east', 'deu_05_decalogue', 'deu_06_shema'):
    ...
    if uid == 'deu_05_decalogue': print('    the step verses:', [b for a, b in st], '| covers DB', VC[CH], '?', [b for a, b in st] == list(range(1, VC[CH] + 1)))
print('  the units\' claim prefixes present anywhere?', {p: sum(1 for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/*_claims.json') + glob.glob(f'{ROOT}/logic/units/*.yaml') if f'{p}-' in open(f, encoding='utf-8').read()) for p in ('DV05A', 'DV05B', 'DV05', 'DV04B')})
print('  existing manifests deu_05*:', sorted(os.path.basename(f) for f in glob.glob(f'{ROOT}/logic/oral_audit/manifests/deu_05*')), '| ledgers deu_*:', sorted(f for f in LED if f.startswith('deu_')))
...
with open(f'{SP}/ch5_store_glosses.txt', 'w', encoding='utf-8') as f:
    for (c, v) in sorted(SG): f.write(f'{c}:{v}: ' + str([(i, t, g) for i, t, g in SG[(c, v)]]) + '\n')
print('  gloss file bytes', os.path.getsize(f'{SP}/ch5_store_glosses.txt'))
OV = open(f'{ROOT}/logic/glosses/word_gloss_overrides.yaml', encoding='utf-8').read()
print('  overrides already for Deut.5:', re.findall(r'"Deut\.5\.[^"]+": "[^"]+"', OV)[:20], '| by_ref last Deut.4 line:', [l for l in OV.split('\n') if l.startswith('  "Deut.4.')][-1:])
