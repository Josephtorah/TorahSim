#!/usr/bin/env python3
# 10b docket (9b's form) — the credited addresses by ledger, and each ledger's row FORM (one sample line per ledger) so the credited verdicts can be carried by script.
import re, os, subprocess, sys
from collections import Counter, defaultdict
ROOT = subprocess.check_output(['git', 'rev-parse', '--show-toplevel'], text=True).strip()
SP = os.path.dirname(os.path.abspath(__file__))
dump = open(f'{SP}/ch12_docket_dump.txt', encoding='utf-8').read()
