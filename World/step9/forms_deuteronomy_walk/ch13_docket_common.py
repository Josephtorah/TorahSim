# THE DEUTERONOMY WALK 11b (10b's form by derive_ch13_docket_tools.py) — the docket parts' shared helper: the dump's addresses in order (the source of truth for coverage — a part can never invent
...
# 'prefix:n' for lo <= n <= hi; a bare string matches one address. The notes name the cell of cold_run_seducers.py (F1 the_header, F2 the_prophet_and_the_test, F3 the_inciter, F4 the_city_heard_of_the_inquiry_and_the_sword,
# F5 the_whole_offering_the_heap_and_the_mercy, F6 the_footer — the design's six cells on the six claims' spans; the_readback the table) on LAW rows, or the callee's cell by CALL.
import os, re
SCR = os.path.dirname(os.path.abspath(__file__))
_dump = open(f'{SCR}/ch13_docket_dump.txt', encoding='utf-8').read()
...
from ch13_credited_rows import CREDITED_SPEC   # 11b: the credited rows carried with their ledgers (ch13_credit_carry.py — 10b's form); a part prepends it to its SPEC
