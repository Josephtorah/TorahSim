#!/usr/bin/env python3
# THE DEUTERONOMY WALK sitting 6b — THE COMPILE OF CHAPTER 8 (2026-09-18): THE EXAM DOCKET'S SCAN, sized by script before the docket is written
# (ch7_docket_scan.py's form on chapter 8). (1) THE LINK ROWS: every segment of the local shelf's Babylonian Talmud, Mishnah and Tosefta exports whose
# English cites a verse of Deuteronomy 8 (the export's chapter 8 = the DB's — the identity, asserted at the reading); (2) THE TOPIC ROWS by address
...
mishnah_chapters('Mishnah_Berakhot', [6, 7])
mishnah_chapters('Mishnah_Bikkurim', [1])
RANGES = [('Berakhot', '5a', '5a'), ('Berakhot', '20b', '21a'), ('Berakhot', '35a', '35b'), ('Berakhot', '41a', '44a'), ('Berakhot', '48b', '49b'), ('Yoma', '74b', '76a'),
          ('Sotah', '4b', '5a'), ('Makkot', '13b', '13b'), ('Eruvin', '96a', '96a')]
...
# THE AMUDIM OF Berakhot 35a-49b SIZED ONE BY ONE (the blessings chapters; the design chooses which are read whole; the rest enumerated outside declared scope)
TB = json.load(open(os.path.join(R, 'Berakhot', 'en.json'), encoding='utf-8'))['text']
print('BERAKHOT 35a-49b BY AMUD (segments; the first 90 characters of each amud):')
for i in range(idx('35a'), idx('49b') + 1):
    print('   %4s %3d  %s' % ('%d%s' % (i // 2 + 1, 'ab'[i % 2]), len(TB[i]), strip(TB[i][0])[:90].replace('\n', ' ') if TB[i] else ''))
READ_AMUDIM = {i for w, a, z in RANGES if w == 'Berakhot' and idx(a) >= idx('35a') for i in range(idx(a), idx(z) + 1)}
_whole = sum(len(TB[i]) for i in range(idx('35a'), idx('49b') + 1)); _read = sum(len(TB[i]) for i in sorted(READ_AMUDIM))
print('ENUMERATED OUTSIDE DECLARED SCOPE — Berakhot 35a-49b whole: %d rows; the amudim read here: %d' % (_whole, _read))
print('  the remainder by amud (rows): %s' % [('%d%s' % (i // 2 + 1, 'ab'[i % 2]), len(TB[i])) for i in range(idx('35a'), idx('49b') + 1) if i not in READ_AMUDIM])
TY = json.load(open(os.path.join(R, 'Yoma', 'en.json'), encoding='utf-8'))['text']
print('YOMA 74b-76a BY AMUD: %s' % [('%d%s' % (i // 2 + 1, 'ab'[i % 2]), len(TY[i])) for i in range(idx('74b'), idx('76a') + 1)])

prior = {}
own = {'deu_08_ekev_2026-09-18.md'}
...
print('  the Mishnah Berakhot / Bikkurim rows credited: %s' % sorted(a for a in prior if a.startswith('Mishnah Berakhot') or a.startswith('Mishnah Bikkurim')))
