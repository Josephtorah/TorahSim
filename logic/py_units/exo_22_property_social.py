#!/usr/bin/env python3
# =============================================================================
# exo_22_property_social — 22:1-30
# PYTHON RENDERING — GENERATED from the FROZEN YAML by render_unit_py.py.
# The YAML (logic/units/exo_22_property_social.yaml) is CANONICAL (Pre-
# Code); this file is a derived, runnable rendering. Do not edit —
# regenerate. The assertion block at the bottom is baked from the Stage D
# interpreter's actual final state: running this file re-proves the unit.
# Experimental model — not binding religious law.
# =============================================================================
"""Theft deposit injury; social protect; firstfruits (22:1–30)"""
...
m.witness_read("burglar_clause", "sun_as_peace_and_the_life_override_seed",
                cites=["Mekhilta on Exod 22:1-2", "Onkelos Exod 22:2"])
# witness-tier presupposed read: babylonian_layer on burglar_clause — read,
# not installed
...
# three_keeper_passages — read, not installed
...
m.witness_read("keeper_oath_clause", "judges_three_and_partial_admission",
                cites=["Mekhilta on Exod 22:6-8", "Onkelos Exod 22:7-8"])
# witness-tier presupposed read: exam_legs_f017 on keeper_oath_clause —
# read, not installed
...
# — read, not installed
m.witness_read("oath_clause", "all_torah_oaths_by_the_name",
                cites=["Mekhilta on Exod 22:9-12"])
# witness-tier presupposed read: administration_and_scope on oath_clause —
# read, not installed
...
m.witness_read("borrower_clause", "four_keepers_table_closes",
                cites=["Mekhilta on Exod 22:13-14"])
# witness-tier presupposed read: the_fourth_role_on_the_hire_clause on
# renter_routing — read, not installed
...
# read, not installed
m.witness_read("seducer_clause", "mohar_is_the_ketubah",
                cites=["Mekhilta on Exod 22:15-16"])
# witness-tier presupposed read: the_constant_in_another_book on
# dowry_pointer — read, not installed
...
m.witness_read("capital_clauses", "the_three_capital_clauses_derivation_layer",
...
m.witness_read("fullness_clause", "gift_order_and_the_standing_act",
...
# not installed
m.witness_read("torn_flesh_clause", "terefah_in_limb_vocabulary_and_the_dogs_wage",
                cites=["Mekhilta on Exod 22:30", "Onkelos Exod 22:30"])
...
if __name__ == "__main__":
    ...
    assert m.WITNESS_READS[1]["cites"] == ['Sanhedrin 72b:1', 'Sanhedrin 72b:5', 'Sanhedrin 72b:6', 'Sanhedrin 72b:11']
    assert all('babylonian_layer' not in f for f in m.WORLD["facts"])
    assert 'burglar_clause' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[2]["cites"] == ['Mekhilta on Exod 22:4']
    assert all('tooth_category_and_the_domain_matrix' not in f for f in m.WORLD["facts"])
    assert 'grazing_clause' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[3]["cites"] == ['Mekhilta on Exod 22:5']
    assert all('accident_as_intent_and_woman_as_man' not in f for f in m.WORLD["facts"])
    assert 'fire_clause' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[4]["cites"] == ['Bava Metzia 94b:6', 'Bava Metzia 94b:8', 'Bava Metzia 94b:9', 'Bava Metzia 94b:10']
    assert all('the_hidden_parameter_diff' not in f for f in m.WORLD["facts"])
    assert 'three_keeper_passages' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[5]["cites"] == ['Mekhilta on Exod 22:6-8', 'Onkelos Exod 22:7-8']
    assert all('judges_three_and_partial_admission' not in f for f in m.WORLD["facts"])
    assert 'keeper_oath_clause' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[6]["cites"] == ['Mekhilta on Exod 22:24', 'Mekhilta on Exod 22:19', 'Mekhilta on Exod 22:27', 'Mishnah Bava Metzia 5:11', 'Mishnah Sanhedrin 7:6', 'Mishnah Shevuot 4:13', 'Mishnah Ketubot 3:9']
    assert all('exam_legs_f017' not in f for f in m.WORLD["facts"])
    assert 'keeper_oath_clause' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[7]["cites"] == ['Mekhilta on Exod 22:9-12']
    assert all('all_torah_oaths_by_the_name' not in f for f in m.WORLD["facts"])
    assert 'oath_clause' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[8]["cites"] == ['Shevuot 39b:7', 'Shevuot 43a:1', 'Shevuot 42b:16', 'Shevuot 42a:12', 'Shevuot 42b:15']
    assert all('administration_and_scope' not in f for f in m.WORLD["facts"])
    assert 'oath_clause' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[9]["cites"] == ['Bava Metzia 94b:15', 'Bava Metzia 94b:19']
    assert all('the_missing_column_generated' not in f for f in m.WORLD["facts"])
    assert 'loss_by_a_fortiori' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[10]["cites"] == ['Mekhilta on Exod 22:13-14']
    assert all('four_keepers_table_closes' not in f for f in m.WORLD["facts"])
    assert 'borrower_clause' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[11]["cites"] == ['Mishnah Bava Metzia 7:8', 'Bava Metzia 93a:17']
    assert all('the_fourth_role_on_the_hire_clause' not in f for f in m.WORLD["facts"])
    assert 'renter_routing' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[12]["cites"] == ['Mekhilta on Exod 22:15-16']
    assert all('mohar_is_the_ketubah' not in f for f in m.WORLD["facts"])
    assert 'seducer_clause' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[13]["cites"] == ['Ketubot 29b:3', 'Mishnah Ketubot 3:4']
    assert all('the_constant_in_another_book' not in f for f in m.WORLD["facts"])
    assert 'dowry_pointer' not in m.WORLD["witnessed"]
    assert m.WITNESS_READS[14]["cites"] == ['Sanhedrin 67a:19', 'Sanhedrin 67a:20', 'Sanhedrin 67a:21', 'Sanhedrin 60a:1', 'Yevamot 4a:4', 'Yevamot 4a:5', 'Yevamot 4a:6', 'Berakhot 21b:9', 'Berakhot 21b:10', 'Berakhot 21b:11', 'Sanhedrin 54b:5', 'Sanhedrin 54b:6', 'Sanhedrin 54b:7', 'Sanhedrin 60b:8', 'Sanhedrin 60b:9', 'Sanhedrin 60b:13', 'Avodah Zarah 51a:13', 'Avodah Zarah 51a:14', 'Sanhedrin 63a:15', 'Sanhedrin 63a:16', 'Sanhedrin 63a:17', 'Sanhedrin 61a:15', 'Sanhedrin 61a:16']
    assert all('the_three_capital_clauses_derivation_layer' not in f for f in m.WORLD["facts"])
    assert 'capital_clauses' not in m.WORLD["witnessed"]
