...
- THE COST RULES (2026-09-16): the bill is TURNS and CONTEXT. NEVER POLL a background job (the harness notifies). The gates ONE chain
  (`sh World/step9/gates_chain.sh <out_dir>`, the SUMMARY read once; CUT 2026-09-19 to ~18 min, GATES_CHAIN.md); ONE fast check before the tape (`checkpoint_check.py --all`); the
...
## 5. THE SITTING SHAPES (the long forms: the addenda's section 5; the newest instances: the map's "Sitting 9" and "Sitting 8b")
...
- Deuteronomy's sittings: the map; the addenda §31-51 (§39 the whole-row rule). The cost cuts: §35, §45, §47.
