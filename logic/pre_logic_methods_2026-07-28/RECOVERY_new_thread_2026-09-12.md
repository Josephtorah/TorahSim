...
NUMBERS, never portion names; decisions ONE AT A TIME. A clean point is announced at every run's end. The mission: the 24 books are the PROGRAM, the Talmud holds the COMPILE RULES, the Mishnah is the ANSWER SHEET; ONE world runs on a
...
## 2. WHERE IT STANDS (2026-09-19, after sitting 7 and THE GATES CUT; the state doc #197 addendum 1 the newest)
- NUMBERS CLOSED. DEUTERONOMY 1-8 COMPILED AND ON THE TAPE (1-7 at 29c189b PUSHED; 8 at sittings 6/6b); 9 READ (sitting 7).
- 223 frozen units, standing 2221, hash 8b8fff1fa28953af. 63 runners, 68 daemons, 466 functions; 1133 kinds / 1031 effects.
- THE TAPE at RUN (1310, 96, 88, 0, 12, 1605, 39, 319, pairs, 127), markers 167, closes 127; the sweep 63/63; every gate GREEN; the
  register gate DECLARED 100 / DEBT 0. The readback's FIFTH form (6b): a STATE graded SUPPLIED, no write — his decision open.
...
- THE COST RULES (2026-09-16): the bill is TURNS and CONTEXT. NEVER POLL a background job (the harness notifies). The gates ONE chain
  (`sh World/step9/gates_chain.sh <out_dir>`, the SUMMARY read once; CUT 2026-09-19 to 18 min 27 s — the snapshot, the parallel probes, the
  stamped sweep, the positions by eight workers; GATES_CHAIN.md); ONE fast check before the tape (`checkpoint_check.py --all`); the
  records from the sheet in ONE call; independent reads batched. ⚠ A COMPILE SITTING IS TWO RUNS, a clean compaction point after each (owner-ruled
  2026-09-18; the four-run rule superseded): RUN A the rereads, the design, the docket; RUN B the types, the runner, the tape to 10/10, the
  chain, the records, the forms — each near 300k, never 600k; a docket past ~700 rows its own run; a READING sitting ONE run; a prior form by
  sed, read where it differs.
...
- THE GATES READ LITERALS: the daemon gate literal submits; `import cold_run_X` in the sequence file the live edge; the token census demands
  edges the runner never imports (seven kinds, each a why and a link); a retype covers every line of a literal.
...
COMPILE_DEBT.md, RECORD_FORMS.md (the records sheet), cold_run_sequence.py (THE TAPE), cold_run_<span>.py (63 runners;
cold_run_good_land.py the newest form), the registries (*_vocabulary.yaml, *_dispositions.yaml, calendar_parameters, population_schema),
the gates (gates_chain.sh runs them all; GATES_CHAIN.md the design; sweep_stamp.json the sweep's stamp), the
probes (*_probes.py), checkpoint_check.py, world_stepper.py, world_board.py, forms_deuteronomy_walk/ (the walk's forms — derive by sed). World/journal/data/world.sqlite the one database; Data/tanakh.sqlite the text store. Memory: <memory>/ — MEMORY.md the
index; deuteronomy-walk.md; cost-rules-no-polling.md; step9-exam-era.md.

## 5. THE SITTING SHAPES (the long forms: the addenda's section 5; the newest instances: the map's "Sitting 7" and "Sitting 6b")
...
with THE REST → `gates_chain.sh` (one summary) → the records from the sheet in one call — the two runs' edge after the docket.
...
- THE LOOP (the sink, the index, installation, the cursor, scenarios, the readback, the stepper, the port, the board, the gates cut, D1-D37): THE_LOOP.md.
...
- Deuteronomy's sittings: the map; the addenda §31-46 (§39 the whole-row rule). The cost cuts: §35, §45.
