...
PENDING AT THREAD-KILL (2026-08-09 ~15:30, owner restarting thread):
(1) RUN FWD-4 NOT YET COMMITTED — tree holds only FWD-4 material +
the three owner-ruled exclusions (grok-mockups/, elijah_docket
gitlink, open_ledger/); commit+push on owner word, then announce
the compaction point. (2) WEB APP (discovered this session):
dev_server.py runs on localhost:8011 (PID was 65486) — serves
web/scroll/ (the Torah scroll app) at "/" and the unit pages at
"/units/UNIT_INDEX.html"; its data layer web/scroll/data/*.json is
regenerated by the chain index_units.py -> index_triage.py ->
export_web.py (~30s, derived-artifact-only, "regenerate at will"
per the script header; the app's own ⟳ button calls the same
chain). freeze_ritual runs index_units.py but NOT export_web.py,
so the scroll app goes stale after freezes: data was regenerated
~14:02 (exo_01-era — Genesis current there) but exo_02-05 are
MISSING from the scroll data. I proposed running the chain; the
owner REJECTED the command (thread ending) — so the regen is
STILL PENDING: run the three-script chain (or let the owner press
⟳ in the app) after resume, on owner word. Unit pages themselves
(UNIT_INDEX + all 81 UNIT_*.html) are fully current locally;
Genesis current on origin through 520c992; Exodus pages local-only
until the FWD-4 commit.
