...
One-call pre-stage for a derivation span. Emits, in one pass:
  1. span shape: verse count, token count, etnachta-less verses with
     their fallback split points
  2. compact token map (idx.translit, etnachta starred, ketiv marked)
...
def main():
    ...
    ch1, v1, ch2, v2 = span
    db = sqlite3.connect(str(REPO / "torah_grok.SNAPSHOT-main-51801ca.sqlite"))
    db.execute("ATTACH ? AS dm",
               (str(REPO / "debut_map.SNAPSHOT-main-51801ca.sqlite"),))
    if ch1 == ch2:
