#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""law03_dump.py — batch reader for LAW ERA block 3 (Exod 21:28-36).
Dumps the NEXT unread chain-readable listings from law03_queue.json,
logging each to the shared chapter ledger (the ledger is the cursor).
He preferred (En only when He missing) to keep volume sane.

  law03_dump.py                # next batch, default cap
  law03_dump.py --work "Rashi" # only listings of works containing str
  law03_dump.py --cap 30000    # char cap per call
  law03_dump.py --status       # remaining counts per work, no dump
  law03_dump.py --tanakh       # dump the TANAKH-VERSE cross-refs
"""
import json
import re
import sqlite3
...
SCRATCH = "<scratch>"
DB = SCRATCH + "/law03_texts.sqlite"  # indexed side DB (FTS5 workaround)
TANAKH_DB = "<repo-old>/elijah_docket/tanakh.sqlite"
QUEUE = SCRATCH + "/law03_queue.json"
LEDGER = "<repo-old>/logic/oral_audit/ledgers/Exod_21.jsonl"
SEEN = SCRATCH + "/law03_seen_segs.txt"
