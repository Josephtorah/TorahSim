#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""law01_dump.py — batch reader for the gen_08 read-only check.
Dumps the NEXT unread chain-readable listings from law01_queue.json,
logging each to the scratchpad ledger (the ledger is the cursor).
He preferred (En only when He missing) to keep volume sane.

  law01_dump.py                # next batch, default cap
  law01_dump.py --work "Rashi" # only listings of works containing str
  law01_dump.py --cap 30000    # char cap per call
