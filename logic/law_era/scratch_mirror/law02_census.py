#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""LAW ERA block 2 census — Exod 21:12-27 (homicide-injury laws), FTS5-safe.
...
rows = db.execute(
    """SELECT DISTINCT source_ref, source_work, category
       ...
       AND anchor_verse BETWEEN 12 AND 27""").fetchall()
...
print("Exod 21:12-27: %d distinct listings | %s" % (
...
side = sqlite3.connect(SCRATCH + "/law02_texts.sqlite")
...
queue = {sr: {"work": sw, "cat": cat, "klass": k, "segs": d}
         for sr, (sw, cat, k, d) in cls.items()}
with open(SCRATCH + "/law02_queue.json", "w", encoding="utf-8") as f:
