...
# Run:  python3 logic/law_era/tanakh_run/app.py
...
LAW_ERA = os.path.dirname(HERE)
ROOT = os.path.dirname(os.path.dirname(LAW_ERA))
TANAKH_DB = os.path.join(ROOT, "elijah_docket", "tanakh.sqlite")
SNAPSHOT_DB = os.path.join(ROOT, "torah_grok.SNAPSHOT-main-51801ca.sqlite")
CATALOG = os.path.join(HERE, "scene_catalog_tanakh_run_2026-08-12.json")
...
M = _load(os.path.join(LAW_ERA, "exo_21_v2_DRAFT.py"), "exo_21_v2")
...
# ---------------------------------------------------------------------------
# THE LEXICON — lemma-bridge: Torah SNAPSHOT glosses keyed by Strong's
# number, applied to any Tanakh verse via tanakh.sqlite's lemma column.
# ---------------------------------------------------------------------------
...
def _build_lexicon():
    """Two layers from the Torah SNAPSHOT: Strong's-number -> gloss (the
    lemma bridge into all 24 books) and unpointed-spelling -> gloss (the
    skeleton fallback for words the lemma misses)."""
    lex, skel = {}, {}
    db = sqlite3.connect(SNAPSHOT_DB)
...
LEXICON, SKELETON = _build_lexicon()

# hand supplement, keyed by unpointed skeleton (wins over the lemma gloss)
SUPPLEMENT = {
    u"\u05d0\u05e8\u05d1\u05e2\u05ea\u05d9\u05dd": "fourfold",          # arbatayim
    u"\u05d9\u05e9\u05dc\u05dd": "he-shall-repay",                      # yeshallem
    u"\u05d5\u05d9\u05e9\u05dc\u05dd": "and-he-shall-repay",
    u"\u05e9\u05d1\u05e2\u05ea\u05d9\u05dd": "sevenfold",               # shivatayim
}
